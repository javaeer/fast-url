...
> ⚠️ **安全警示**：当前 `src/main/resources/application.yml` 中**明文硬编码了真实服务器 IP 与数据库 / Redis / RabbitMQ 凭据**。在公开仓库前请务必先轮转这些凭据，并将其改为环境变量 / 配置中心注入。详见 [安全须知](#🔐-安全须知) 与 [SECURITY.md](SECURITY.md)。
